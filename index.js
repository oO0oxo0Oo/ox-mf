...
class Controls {
	...
	getPiecePosition(piece) {
		...
		return this.game.cube.object
			.worldToLocal(position.sub(this.game.cube.animator.position))
			.round();
	}
...
class Scrambler {
	...
	scramble(scramble) {
		...
		if (this.moves.length < 1) {
			...
			const modifiers = ["", "'", "2"];
			const total =
				typeof scramble === "undefined" ? this.scrambleLength : scramble;
...
class Scores {
	...
	getAverage(count) {
		...
		return this.convertTime(
			this.scores.slice(-count).reduce((a, b) => a + b, 0) / count
		);
	}
...
class Themes {
	...
	setTheme(theme) {
		...
		this.game.dom.buttons.pwa.style.color =
		...
		this.dom.game.onclick = (event) => {
			...
			if (this.state === MENU) {
				...
				const duration = this.saved
					? 0
					: this.scrambler.converted.length * this.controls.flipSpeeds[0];
