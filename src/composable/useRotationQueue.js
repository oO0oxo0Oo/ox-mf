...
export function useRotationQueue() {
  ...
  // 计算属性
  const queueLength = computed(() => animationQueue.length)
  const hasCurrentRotation = computed(() => currentRotationState !== null)
  const getAnimationEngineStatus = computed(() => animationEngine.getStatus())
  ...
  // 设置控制实例
  function setControlsInstance(controls) {
